<?php
/**
 * Template Name: MBC Page — Full Hero
 * Template Post Type: page
 *
 * Option A: Full-bleed hero image, page title centred over it.
 * Dramatic, suits landing-style pages (About, Church Life etc.)
 */
?><!DOCTYPE html>
...
/* ── FULL HERO PAGE TEMPLATE ── */
body.mbc-page-full {
  background:#111; color:rgba(255,255,255,.78);
  display:flex; flex-direction:column; min-height:100vh;
}
.mbc-ph-hero, .mbc-ph-breadcrumb,
.mbc-ph-content-wrap { width:100%; flex-shrink:0; }
.mbc-footer { width:calc(100% - 220px); margin-left:220px; flex-shrink:0; box-sizing:border-box; }
...
.mbc-ph-nav-links li a:hover,
.mbc-ph-nav-links li.current-menu-item a { color: #c9a84c; }
...
.mbc-ph-breadcrumb a:hover { color: #c9a84c; }
.mbc-ph-breadcrumb span { margin: 0 6px; }
...
.mbc-ph-content-wrap h1,
.mbc-ph-content-wrap h2 {
  font-family: 'Playfair Display', serif;
  color: #fff;
  font-weight: 700;
  line-height: 1.2;
...
.mbc-ph-content-wrap h1 { font-size: 2rem; }
.mbc-ph-content-wrap h2 { font-size: 1.5rem; margin-top: 48px; }
.mbc-ph-content-wrap h2 em { color: #c9a84c; font-style: italic; font-weight: 400; }
.mbc-ph-content-wrap h3 {
  font-family: 'Outfit', sans-serif;
  color: #c9a84c; font-size: 1rem;
  font-weight: 700; letter-spacing: .04em;
  text-transform: uppercase; margin: 36px 0 12px;
}
.mbc-ph-content-wrap p {
  font-family: 'Lato', sans-serif;
  font-size: 1rem; line-height: 1.82;
  color: rgba(255,255,255,.62);
  margin-bottom: 20px; font-weight: 300;
}
.mbc-ph-content-wrap a {
  color: #c9a84c; text-decoration: none;
  border-bottom: 1px solid rgba(201,168,76,.3);
  transition: border-color .2s;
}
.mbc-ph-content-wrap a:hover { border-color: #c9a84c; }
.mbc-ph-content-wrap blockquote {
  border-left: 3px solid #c9a84c;
  margin: 32px 0; padding: 4px 0 4px 24px;
}
.mbc-ph-content-wrap blockquote p {
  font-family: 'Playfair Display', serif;
  font-style: italic; font-size: 1.15rem;
  color: rgba(255,255,255,.55); font-weight: 400;
}
.mbc-ph-content-wrap img {
  max-width: 100%; height: auto;
  margin: 24px 0;
}
.mbc-ph-content-wrap ul,
.mbc-ph-content-wrap ol {
  font-family: 'Lato', sans-serif;
...
.mbc-ph-content-wrap li { margin-bottom: 6px; }
.mbc-ph-content-wrap hr {
  border: none; border-top: 1px solid #2a2a2a;
  margin: 48px 0;
}

/* Elementor content inside this template */
.mbc-ph-content-wrap .elementor { width: 100%; }

@media (max-width: 768px) {
  .mbc-ph-nav { padding: 0 20px; }
  .mbc-ph-nav-links { display: none; }
  .mbc-ph-hero { height: 50vh; }
  .mbc-ph-content-wrap { padding: 40px 24px 60px; }
  .mbc-ph-breadcrumb { padding: 10px 24px; }
}
</style>
...
<!-- NAV -->
<nav class="mbc-ph-nav">
  <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="mbc-ph-nav-logo">
...
<!-- HERO -->
<div class="mbc-ph-hero">
  <div class="mbc-ph-hero-bg" <?php
    $thumb = get_the_post_thumbnail_url( get_the_ID(), 'full' );
    if ( $thumb ) echo 'style="background-image:url(' . esc_url( $thumb ) . ')"';
  ?>></div>
  <div class="mbc-ph-hero-overlay"></div>
  <div class="mbc-ph-hero-content">
    <span class="mbc-ph-eyebrow">Moortown Baptist Church</span>
    <h1 class="mbc-ph-title"><?php the_title(); ?></h1>
    <?php if ( has_excerpt() ) : ?>
      <p class="mbc-ph-subtitle"><?php the_excerpt(); ?></p>
    <?php endif; ?>
    <div class="mbc-ph-rule"></div>
  </div>
</div>

<!-- BREADCRUMB -->
<div class="mbc-ph-breadcrumb">
